...
<div>
    <template>
        <v-layout row wrap class="pt-4 pl-4 pr-4">
            <v-flex xs6>
                <v-dialog ref="dialog" v-model="modal" :return-value.sync="month" persistent lazy full-width width="290px">
                    ...
                    <v-date-picker v-model="month" type="month" scrollable color="green">
                        <v-spacer></v-spacer>
                        <v-btn flat color="primary" @click="modal = false">Cancel</v-btn>
                        <v-btn flat color="primary" @click="onMonthSelected">OK</v-btn>
                    </v-date-picker>
                </v-dialog>
            </v-flex>
            <v-flex xs6>
    ...
    <template>
        <div id="printMonthly">
            <center v-show="isPrint">
                ...
                <h1>({{month | moment('MMMM, YYYY')}})</h1>
            </center>
            <v-layout column class="pr-4 pb-5">
                <v-flex xs12>
                    <template v-if="monthlySales != null && monthlySales.length > 0">
                        <v-list two-line>
                            <template v-for="(sales, index) in monthlySales">
                                ...
                                <v-divider :inset="true" v-if="index + 1 < monthlySales.length" :key="index"></v-divider>
                            </template>
                        </v-list>
                        <template>
                            ...
                            <v-list-tile avatar>
                                ...
                                <v-list-tile-action>
                                    <p class="title font-weight-meduim red--text mt-3">
                                        ₱ {{ Number(getTotalMonthlySales).toLocaleString() }}
                                    </p>
                                </v-list-tile-action>
                            </v-list-tile>
                        </template>
                    ...
                    <template v-else>
                        <div class="text-xs-center py-3 mb-5">
                            ...
                            <div class="subheading">No sales on this month.</div>
                        </div>
                    </template>
                </v-flex>
            </v-layout>
    ...
    export default {
        ...
        methods: {
            onMonthSelected() {
                ...
                this.$store.dispatch('sales/fetchMonthlySales', this.month)
            },
            printDialog() {
                this.isPrint = true
                setTimeout(() => {
                    this.$htmlToPaper('printMonthly');
                    this.isPrint = false
                }, 500);
            }
        },
        computed: {
            getMonthlySales() {
                return this.$store.getters['sales/getMonthlySales']
            },
            getTotalMonthlySales() {
                return this.$store.getters['sales/getTotalMonthlySales']
            },
            computedDateFormattedMomentjs () {
                return this.month ? moment(this.month).format('MMMM, YYYY') : ''
            }
        },
        watch: {
            getMonthlySales(val) {
                if (val !== null && val !== undefined) {
                    this.monthlySales = val
                }
            }
        },
        created() {
